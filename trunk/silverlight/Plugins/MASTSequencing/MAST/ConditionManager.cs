...
namespace org.OpenVideoPlayer.Advertising.MAST {
	...
	public class ConditionManager : IDisposable {
		...
		public void DoOnEventFired() {
			OnEventFired(this, new EventArgs());
		}
			...
			switch (typeName) {
				...
				default:
					if (prop.GetType().IsPrimitive) {
						...
						//unknown type
						throw new Exception(string.Format("The property type '{1}' is unknown. ", prop.GetType().Name));
					}
					break;
			}
		}
		...
		private bool ModTimeSpans(TimeSpan prop, TimeSpan val) {
			...
			if (!modTimerSet && prop > TimeSpan.FromMilliseconds(100)) {
				...
				Async.SetTimer(DoOnEventFired, "TimeModTimer", ReenteranceMode.Bypass, prop, TimeSpan.FromMilliseconds(-1));
				modTimerSet = true;
			}
		...
		private bool CompareBool(object prop, string val) {
			switch (Condition.@operator) {
				case Operator.EQ:
					//TODO - make option for case sensitivity?  for now stick to ignoring case
					return ((bool)prop == Conversion.ParseBool(val));
				case Operator.NEQ:
					return ((bool)prop != Conversion.ParseBool(val));
				default:
					//unsupported operator
		...
		public void Dispose() {
			...
			if(eventHandler!=null) {
				ReflectionHelper.DetachEvent(this, eventHandler);
			}
		}
	}
}