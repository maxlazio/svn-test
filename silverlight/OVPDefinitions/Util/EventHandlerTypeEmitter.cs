...
namespace CommonEventHandler
{
	...
	{
		private static Dictionary<string, Type> handlerTypes = new Dictionary<string, Type>();  
		string assemblyName;
		...
		public Type GetEventHandlerType(EventInfo Info) 
		{
			...
			Type tpEventHandler = handlerTypes.ContainsKey(handlerName)? handlerTypes[handlerName] : null;
		...
		private Type EmitHelperClass(string HandlerName, EventInfo Info) 
		{
			...
			if (raiseEventMethod== null) throw new Exception("CommonEventHandlerDlg:Invoke not found");
			...
			if (combineMtd == null) throw new Exception("Delegate:Combine not found");
			...
			if (removeMtd == null) throw new Exception("Delegate:Remove not found");
